 ...
 * The Oral Objective option is appended dynamically when an oral-eligible
 * subject (English Language) is selected; see buildQuestionTypes().
 */
...
/**
 * Returns the full question type list for the currently selected subject.
 * Inserts the ORAL_ENG_OBJ option between OBJ and THEORY when the subject
 * is English Language (is_oral_eligible flag from the API).
 */
function buildQuestionTypes(isOralSubject) {
  if (!isOralSubject) return BASE_QUESTION_TYPES;
  return [
    { value: "",             label: "All Types"           },
    { value: "OBJ",          label: "Objective Only"      },
    { value: "ORAL_ENG_OBJ", label: "Oral Objective"      },
    { value: "THEORY",       label: "Theory Only"         },
  ];
}
...
const formStyles = ` 
  ...
  .access-banner a { color: inherit; font-weight: 700; text-decoration: underline; }
  ...
  .form-label .hint { font-weight: 400; font-style: italic; color: var(--muted); }
  ...
  /* Type toggle — flex, grows to fit 3 or 4 options */
  .type-toggle { display: flex; gap: 0.375rem; flex-wrap: wrap; }
  .type-btn {
    flex: 1; min-width: 80px; padding: 0.55rem 0.5rem; border-radius: 8px;
...
export default function QuestionGeneratorForm({ onResults, onClear, access }) {
  ...
  // Whether the currently selected subject supports ORAL_ENG_OBJ questions.
  // Derived from is_oral_eligible on the subject object returned by the API.
  const selectedSubjectObj = subjects.find(s => String(s.id) === String(form.subject));
  const isOralSubject      = selectedSubjectObj?.is_oral_eligible ?? false;

  // Available question types — expands when English is selected
  ...
  useEffect(() => {
    if (!isOralSubject && form.question_type === 'ORAL_ENG_OBJ') {
      ...
      <form className="gen-form" onSubmit={handleSubmit}>
        ...
        {access && isFree && !isBlocked && (
          <div className="access-banner free">
            ...
            <a href="/pricing/" style={{
              ...
              borderRadius: "7px", color: "var(--gold)", fontWeight: 700, fontSize: "0.75rem",
        ...
        {access && isBlocked && (
          <div style={{
            ...
            border: "1px solid rgba(245,200,66,0.3)", borderRadius: "10px",
            padding: "1rem 1.1rem", marginBottom: "1.25rem",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.75rem" }}>
              <span style={{ fontSize: "1.1rem" }}>🔒</span>
              <div>
                <div style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: "0.88rem", color: "var(--gold)", marginBottom: "0.2rem" }}>
            ...
            <a href="/pricing/?tab=teacher" style={{
              display: "block", width: "100%", padding: "0.6rem",
              background: "var(--gold)", color: "#ffffff", borderRadius: "8px",
              ...
              <div className="selected-tags" style={{ marginTop: "0.6rem" }}>
                {form.years.map((y) => (
                  <span key={y} className="selected-tag">
                    {y}
                    <button type="button" onClick={() => toggleMulti("years", y)}>×</button>
                  </span>
                ))}
              </div>
            )}
        ...
        <div className="form-section">
          ...
          <div className="form-group">
            ...
            {topics.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem", maxHeight: "180px", overflowY: "auto", padding: "0.25rem" }}>
                {topics.map((t) => (
                  <label key={t.id} style={{
                    ...
                    <span style={{ fontSize: "0.85rem", color: form.topics.includes(String(t.id)) ? "var(--accent)" : "var(--text)" }}>
                      {t.name}
                    </span>
                  </label>
                ))}
                ...
                {form.topics.map((id) => {
                  ...
                  return t ? (
                    <span key={id} className="selected-tag">
                      {t.name}
                      <button type="button" onClick={() => toggleMulti("topics", id)}>×</button>
                    </span>
                  ) : null;
                })}
              </div>
        ...
        <div className="form-section">
          ...
          <div className="form-group">
            <label className="form-label">
              Question Type
              {/* Hint appears when English is selected to explain the extra option */}
              {isOralSubject && (
                <span className="hint"> — includes Oral for English Language</span>
              )}
            </label>
          ...
          <div className="form-group">
            ...
            <input type="number" className="form-input"
              value={form.num_questions} min={1} max={maxQ}
              style={{ width: "100px" }}
