...
def get_active_subscription(user):
    ...
    return user.active_subscription  # uses cached property on the User model


def has_subscription(user, plan_type=None):
    """
    Returns True if the user has an active subscription of the given type.

    When subscription_required=False in PlatformSettings, always returns True
    so the entire platform becomes free without touching any other code.

    plan_type: 'STUDENT_BASIC' | 'TEACHER_PRO' | None (any active plan)
    """
    # Global free mode — treat every user as subscribed
...
def check_practice_access(user):
    ...
    if getattr(user, 'is_admin', False):
        return {'allowed': True, 'max_questions': 9999, 'sessions_left': 9999, 'is_free': False, 'reason': ''}

    # Subscribed users (or free-mode) — unlimited
    if has_subscription(user, 'STUDENT_BASIC') or has_subscription(user, 'TEACHER_PRO'):
        return {'allowed': True, 'max_questions': 9999, 'sessions_left': 9999, 'is_free': False, 'reason': ''}

    # Free tier — check daily limit from PlatformSettings
    ...
    if not tracker.can_start_session():
        return {
            'allowed':       False,
            'max_questions': ps.free_question_limit,
            'sessions_left': 0,
            'is_free':       True,
            'reason':        (
                f"You've used all {ps.free_daily_sessions} free practice sessions "
                f"for today. Come back tomorrow or upgrade to practise without limits."
            ),
        }
...
def check_test_builder_access(user):
    ...
    """
    if getattr(user, 'is_admin', False):
        return {'allowed': True, 'is_free': False, 'trials_remaining': 9999, 'max_questions': 9999, 'pdf_only': False, 'reason': ''}
    ...
    if not tracker.can_use_test_builder():
        return {
            ...
            'max_questions':    ps.free_question_limit,
            'pdf_only':         True,
            'reason':           "You've used both free test builder trials. Upgrade to Teacher Pro for unlimited access.",
        }
...
def check_lesson_note_access(user, topic):
    ...
    if getattr(user, 'is_admin', False) or has_subscription(user, 'TEACHER_PRO'):
        return {'allowed': True, 'is_free': False, 'already_accessed': False, 'slots_remaining': 9999, 'reason': ''}
