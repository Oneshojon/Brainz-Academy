...
def messaging(request):
    ...
    POST fields:
        ...
        min_recent_sessions int   (optional, students only — last 30 days)
        subject_id          int   (optional, teachers only — test builder subject)
        selected_user_ids   list  (optional, repeated field — individual overrides)
 
    Recipient resolution priority:
        1. If selected_user_ids present → send only to those users (individual mode)
        2. Otherwise → resolve_recipients(filter_params) (audience mode, existing behaviour)
    ...
    # ── Resolve recipients ────────────────────────────────────────────────────
    # Individual mode: admin hand-picked specific users via the search widget.
    # Audience mode:   fall back to resolve_recipients() as before.
    from Users.models import CustomUser
 
    selected_ids = [
        int(i) for i in request.POST.getlist('selected_user_ids') if i.isdigit()
    ]
 
    if selected_ids:
        # Individual mode — validate IDs belong to active non-admin users
        recipient_qs = (
            CustomUser.objects
            .filter(id__in=selected_ids, is_active=True, is_admin=False)
            .only('id', 'email', 'first_name', 'last_name')
        )
        # Store selected mode in filter snapshot for history display
        filter_params['selected_user_ids'] = selected_ids
    else:
        # Audience mode — existing behaviour
        recipient_qs = resolve_recipients(filter_params)
...
def messaging_preview(request):
    ...
    # Individual mode — selected_user_ids overrides audience count
    selected_ids = [i for i in data.get('selected_user_ids', []) if isinstance(i, int)]
    if selected_ids:
        return JsonResponse({
            'count': len(selected_ids),
            'label': f'{len(selected_ids)} selected user{"s" if len(selected_ids) != 1 else ""}',
        })
...
@admin_required
def messaging_users(request):
    """
    GET /teacher/messaging/users/?audience=STUDENTS
    Returns a lightweight JSON list of users in the selected audience scope.
 
    Used by the compose form search widget — fetched once on first focus,
    then filtered client-side. Returns only the fields needed for display.
 
    Excludes admin users and inactive accounts (same as resolve_recipients).
    No caching — admin tool, low traffic, needs fresh data.
 
    Response shape:
        [{"id": 1, "email": "ada@test.com", "name": "Ada Obi"}, ...]
    """
    from Users.models import CustomUser
    ...
    users = list(
        qs.only('id', 'email', 'first_name', 'last_name')
        .order_by('email')
