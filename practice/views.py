...
def past_papers(request):
    """
    Student past papers browser.
    GET ?subject=<id>&paper=<paper_id>
    """
    from catalog.models import Subject, PastPaper
    ...
    if selected_subject_id:
        selected_subject = next(
            (s for s in all_subjects if str(s.id) == str(selected_subject_id)),
            None
        )

    # ── Load all papers for subject — single optimised query ─────────────────
    if selected_subject:
        papers_flat = list(
            PastPaper.objects
            .filter(exam_series__subject=selected_subject)
            .select_related(
                'exam_series',
                'exam_series__exam_board',
                'exam_series__subject',
            )
            .order_by('-exam_series__year', 'exam_series__sitting', 'paper_type')
        )

    # ── Selected paper ────────────────────────────────────────────────────────
    if selected_paper_id and papers_flat:
        selected_paper = next(
            (p for p in papers_flat if str(p.id) == str(selected_paper_id)),
            None
        )
    # Auto-select first paper if subject selected and no paper chosen
    elif papers_flat and not selected_paper_id:
    ...
    return render(request, 'practice/past_papers.html', context)