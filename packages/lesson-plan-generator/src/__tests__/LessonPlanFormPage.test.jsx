...
describe('LessonPlanFormPage', () => {
  ...
  it('creates a draft and navigates to its detail page', async () => {
    ...
    // '42' + { relative: 'path' }, not '/lesson-plans/42' — an absolute
    // path here would resolve from the host BrowserRouter's basename root
    // regardless of where this package is actually mounted (see
    // LessonPlansLayout's docstring). Path-relative resolution against the
    // current ".../lesson-plans/new" URL is what makes this correct under
    // both frontend/ui and schools/ui.
    await waitFor(() =>
      expect(mockNavigate).toHaveBeenCalledWith('42', { relative: 'path' }),
    );
  });
