...
describe('LessonPlanDetailPage', () => {
  ...
  it('navigates back to the list after a successful delete, at a nested mount depth', async () => {
    /**
     * This is the regression test for the exact bug the mount-depth work
     * introduced a fix for: handleDelete used to call
     * navigate('/lesson-plans/'), an absolute path that only happened to
     * work because frontend/ui mounts this package right at its router
     * root. Rendering under an extra "/school-plan/portal/teacher" prefix
     * -- with no route defined at literal "/lesson-plans/" -- proves the
     * '..' + relative:'path' fix actually resolves relative to wherever
     * this page is mounted, not the old hardcoded absolute path.
     */
    getLessonPlan.mockResolvedValue(DRAFT_PLAN);
    deleteLessonPlan.mockResolvedValue({});

    render(
      <MemoryRouter initialEntries={['/school-plan/portal/teacher/lesson-plans/7']}>
        <Routes>
          <Route path="/school-plan/portal/teacher/lesson-plans/" element={<div>List Page Stand-in</div>} />
          <Route path="/school-plan/portal/teacher/lesson-plans/:id" element={<LessonPlanDetailPage />} />
        </Routes>
      </MemoryRouter>,
    );
    ...
    await waitFor(() => screen.getByRole('button', { name: /yes, delete/i }));
    fireEvent.click(screen.getByRole('button', { name: /yes, delete/i }));

    await waitFor(() => expect(screen.getByText('List Page Stand-in')).toBeInTheDocument());
    expect(deleteLessonPlan).toHaveBeenCalled();
  });
