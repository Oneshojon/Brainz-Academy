...
/**
 * Shared shell for every .../lesson-plans/* page — topbar + page container.
 * Self-contained styling since the host app's own top-level styles aren't
 * present on this route tree.
 *
 * `logoTo` and `backLinks` are host-supplied rather than hardcoded: a
 * `<Link to="/...">` (absolute path) always resolves from the enclosing
 * BrowserRouter's `basename`, not from wherever this layout happens to be
 * mounted. frontend/ui mounts this package directly under its "/tools"
 * root (so "/" correctly means "back to Test Builder"); schools/ui mounts
 * it one level deeper under "/school-plan/portal/teacher/lesson-plans"
 * (so "/" there would silently point at the wrong place). Each host passes
 * its own values via LessonPlansRouter — see that file's props.
 *
 * @param {string} logoTo - relative `to` target for the logo link.
 * @param {{label: string, to?: string, href?: string}[]} backLinks -
 *   rendered in order; entries with `href` render as a plain <a> (for
 *   linking outside the SPA, e.g. a Django-rendered dashboard), entries
 *   with `to` render as a React Router <Link> (relative navigation).
 */
export default function LessonPlansLayout({ logoTo, backLinks }) {
  return (
    <>
      <style>{styles}</style>
      <div className="lp-shell">
        <header className="lp-topbar">
          <Link to={logoTo} className="lp-logo">
            <img
          ...
          <div className="lp-topbar-right">
            {backLinks.map((link) =>
              link.href ? (
                <a key={link.label} href={link.href} className="lp-btn-back">
                  {link.label}
                </a>
              ) : (
                <Link key={link.label} to={link.to} className="lp-btn-back">
                  {link.label}
                </Link>
              ),
            )}
